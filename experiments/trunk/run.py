...
trunk_kona.ply 내부에 나머지 아이템들을 패킹하고
...
# ── 경로 ──────────────────────────────────────────────────────────────────────
_HERE     = Path(__file__).parent                       # experiments/trunk/
...
SPACE_FILE = "trunk_kona.ply"

# 공간 메시 복셀화 해상도 (긴 축 기준 voxel 수)
# → 이 값이 트렁크와 아이템 사이의 공통 pitch를 결정
...
def run_packing(item_paths: list[Path], space_path: Path):
    """트렁크 내부에 아이템들을 패킹하고 PackingResult를 반환합니다."""
